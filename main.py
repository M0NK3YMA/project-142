...
def recommended_articles():
    ...
    all_recommended_articles=pd.DataFrame(columns=col_names)
    for liked_article in liked_articles:
        output=get_recommendations(liked_article["original_title"])
        all_recommended_articles.append(output)
    all_recommended_articles.drop_duplicates(subset=["original_title"],inplace=True)
    recommended_article_data=[]
    for index,row in all_recommended_articles.iterrows():
        _p={
        "url":row["url"],
        "title": row["title"],
        "text": row["text"] or "N/A",
